...
def defaults():
    ...
    env.service_name = ["gunicorn-onepager"]
    env.code_dir = '/srv/onepager'

@task
def test():
    defaults()
    env.hosts = ['test.your-site.com']
...
def deploy(id=None, silent=False, force=False):
    ...
    package_changed = force or vcs.changed_files(revset, r' onepager/package.json')
    if package_changed:
        print colors.yellow("Will run npm install")

    # See if we have changes in app source or static files
    app_changed = force or vcs.changed_files(revset, [r' onepager/app', r' onepager/static', r' onepager/settings', r'webpack'])
    if app_changed:
    ...
    if crontab_changed:
        with cd(env.code_dir):
            sudo('cp deploy/crontab.conf /etc/cron.d/onepager')
...
def setup_server():
    ...
    # Create database
    sudo('echo "CREATE DATABASE onepager; '
         '      CREATE USER onepager WITH password \'%s\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE onepager to onepager;" '
    ...
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/onepager/settings/local.py', use_sudo=True)

    # Create necessary dirs, with correct permissions
    mkdir_wwwdata('/var/log/onepager/')
    with cd(env.code_dir + '/onepager'), prefix('. ../venv/bin/activate'):
    ...
    sudo('chown -R www-data:www-data /var/log/onepager/')

    # Copy logrotate, nginx, crontab and gunicorn confs
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/onepager')
        sudo('cp deploy/%s /etc/nginx/sites-enabled/onepager' % env.nginx_conf)
        sudo('cp deploy/gunicorn.conf /etc/init/gunicorn-onepager.conf')
        sudo('cp deploy/crontab.conf /etc/cron.d/onepager')
